...
> **Status:** specified, not yet implemented. Every interface below is defined in
> [`architecture.md`](architecture.md); none of it ships in a release yet.
...
| `--version` | flag | — | Print the version, plus the USBGuard version detected on the bus, and exit. |
...
| `tray-notice-shown` | boolean | `false` | Whether the one-time notice about tray availability has been given. |
...
| Session | `org.freedesktop.Notifications` | Desktop notifications. | No — degrades to no notifications. |
